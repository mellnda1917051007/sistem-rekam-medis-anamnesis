...
$file = dirname(__FILE__) . "/settings_keluarga.xml";
$read = simplexml_load_file($file);
$exe = new \SimpleXMLElement($read->asXML());
$rows = count($exe);

class Setting_Keluarga
{
    private $datas = [];
    ...
    {
        $file = dirname(__FILE__) . "/menu_keluarga.xml";
        return new \SimpleXMLElement($file, 0, true);
    }

    public function get_lokasi_lg()
    {
        return base_url("") . "/data/tmp/" . Setting_Keluarga::instance()->lg . "/file";
    }

    public function get_lokasi_tmp()
    {
        return base_url("") . "/data/tmp/" . Setting_Keluarga::instance()->tmp . "/file/";
    }
...
for ($i = 0; $i < $rows; $i++) {
    ...
    if ($exe->users[$i]->id == '1') {
        ...
        Setting_Keluarga::set_data($datas);
    }
}
