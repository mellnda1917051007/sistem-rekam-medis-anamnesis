<?php
$this->extend('home/app.php');

$this->section("halaman");
?>

<section id="slider-part" class="slider-active">
    <div class="single-slider bg_cover pt-150" style="background-image: url(home/data/image/background/slide_a.png)"
         data-overlay="4">
        <div class="container">
            <div class="row">
                <div class="col-xl-7 col-lg-9">
                    <div class="slider-cont">
                        <h1 data-animation="bounceInLeft" data-delay="1s">Selamat Datang Di Website Kami</h1>
                        <p data-animation="fadeInUp" data-delay="1.3s">Semoga pelayanan kami dapat membuat anda puas
                            juga membuat hari pernikahan anda lebih indah dan mengesankan.</p>
                        <ul>
                            <li><a data-animation="fadeInUp" data-delay="1.6s" class="main-btn"
                                   href="<?= base_url('produk') ?>">Paket Event</a></li>
                            <li><a data-animation="fadeInUp" data-delay="1.9s" class="main-btn main-btn-2"
                                   href="<?= base_url('login') ?>">Login</a></li>
                        </ul>
                    </div>
                </div>
            </div> <!-- row -->
        </div> <!-- container -->
    </div> <!-- single slider -->

    <div class="single-slider bg_cover pt-150" style="background-image: url(home/data/image/background/slide_a.png)"
         data-overlay="4">
        <div class="container">
            <div class="row">
                <div class="col-xl-7 col-lg-9">
                    <div class="slider-cont">
                        <h1 data-animation="bounceInLeft" data-delay="1s">Selamat Datang Di Website Kami</h1>
                        <p data-animation="fadeInUp" data-delay="1.3s">Semoga pelayanan kami dapat membuat anda puas
                            juga membuat hari pernikahan anda lebih indah dan mengesankan.</p>
                        <ul>
                            <li><a data-animation="fadeInUp" data-delay="1.6s" class="main-btn" href="#">Read More</a>
                            </li>
                            <li><a data-animation="fadeInUp" data-delay="1.9s" class="main-btn main-btn-2" href="#">Get
                                    Started</a></li>
                        </ul>
                    </div>
                </div>
            </div> <!-- row -->
        </div> <!-- container -->
    </div> <!-- single slider -->

    <div class="single-slider bg_cover pt-150" style="background-image: url(home/data/image/background/slide_a.png)"
         data-overlay="4">
        <div class="container">
            <div class="row">
                <div class="col-xl-7 col-lg-9">
                    <div class="slider-cont">
                        <h1 data-animation="bounceInLeft" data-delay="1s">Selamat Datang Di Website Kami</h1>
                        <p data-animation="fadeInUp" data-delay="1.3s">Semoga pelayanan kami dapat membuat anda puas
                            juga membuat hari pernikahan anda lebih indah dan mengesankan.</p>
                        <ul>
                            <li><a data-animation="fadeInUp" data-delay="1.6s" class="main-btn" href="#">Read More</a>
                            </li>
                            <li><a data-animation="fadeInUp" data-delay="1.9s" class="main-btn main-btn-2" href="#">Get
                                    Started</a></li>
                        </ul>
                    </div>
                </div>
            </div> <!-- row -->
        </div> <!-- container -->
    </div> <!-- single slider -->
</section>

<section id="category-part">
    <div class="container">
        <div class="category pt-40 pb-80">
            <div class="row">
                <div class="col-lg-4">
                    <div class="category-text pt-40">
                        <h2>Aplikasi Wedding Organizer Assyfa Studio</h2>
                    </div>
                </div>
                <div class="col-lg-6 offset-lg-1 col-md-8 offset-md-2 col-sm-8 offset-sm-2 col-8 offset-2">
                    <div class="row category-slied mt-40">
                        <div class="col-lg-4">
                            <a href="<?= base_url('galeri') ?>">
                                    <span class="singel-category text-center color-1">
                                        <span class="icon">
                                            <img src="home/data/tmp/tmp 44/edubin/images/all-icon/ctg-1.png" alt="Icon">
                                        </span>
                                        <span class="cont">
                                            <span>Galery</span>
                                        </span>
                                    </span> <!-- singel category -->
                            </a>
                        </div>
                        <div class="col-lg-4">
                            <a href="<?= base_url('keranjang') ?>">
                                    <span class="singel-category text-center color-2">
                                        <span class="icon">
                                            <img src="home/data/tmp/tmp 44/edubin/images/all-icon/ctg-2.png" alt="Icon">
                                        </span>
                                        <span class="cont">
                                            <span>Pemesanan</span>
                                        </span>
                                    </span> <!-- singel category -->
                            </a>
                        </div>
                        <div class="col-lg-4">
                            <a href="<?= base_url('transaksi') ?>">
                                    <span class="singel-category text-center color-3">
                                        <span class="icon">
                                            <img src="home/data/tmp/tmp 44/edubin/images/all-icon/ctg-3.png" alt="Icon">
                                        </span>
                                        <span class="cont">
                                            <span>Transaksi</span>
                                        </span>
                                    </span> <!-- singel category -->
                            </a>
                        </div>
                        <div class="col-lg-4">
                            <a href="<?= base_url('galeri') ?>">
                                    <span class="singel-category text-center color-1">
                                        <span class="icon">
                                            <img src="home/data/tmp/tmp 44/edubin/images/all-icon/ctg-1.png" alt="Icon">
                                        </span>
                                        <span class="cont">
                                            <span>Galery</span>
                                        </span>
                                    </span> <!-- singel category -->
                            </a>
                        </div>
                        <div class="col-lg-4">
                            <a href="<?= base_url('keranjang') ?>">
                                    <span class="singel-category text-center color-2">
                                        <span class="icon">
                                            <img src="home/data/tmp/tmp 44/edubin/images/all-icon/ctg-2.png" alt="Icon">
                                        </span>
                                        <span class="cont">
                                            <span>Pemesanan</span>
                                        </span>
                                    </span> <!-- singel category -->
                            </a>
                        </div>
                        <div class="col-lg-4">
                            <a href="<?= base_url('transaksi') ?>">
                                    <span class="singel-category text-center color-3">
                                        <span class="icon">
                                            <img src="home/data/tmp/tmp 44/edubin/images/all-icon/ctg-3.png" alt="Icon">
                                        </span>
                                        <span class="cont">
                                            <span>Transaksi</span>
                                        </span>
                                    </span> <!-- singel category -->
                            </a>
                        </div>
                    </div> <!-- category slied -->
                </div>
            </div> <!-- row -->
        </div> <!-- category -->
    </div> <!-- container -->
</section>

<section id="about-part" class="pt-65">
    <div class="container">
        <div class="row">


            <h2>&nbsp;</h2>

        </div>
    </div>

</section>

<?php

$this->endSection();