                    ...
                    <tr class="event2">
                        <td class="th_border cell" align="center" width="200">
                            <table border="0">
                                <tr>
                                    <td>
                                        <a href="<?php index(); ?>/detail?proses=<?= encrypt($data['id_admin']); ?>">
                                            <?php btn_detail('Detail'); ?></a>
                                    </td>
                                    <td>
                                        <a href="<?php index(); ?>/edit?proses=<?= encrypt($data['id_admin']); ?>">
                                            <?php btn_edit('Edit'); ?></a>
                                    </td>
                                    <td>
                                        <a onclick="hapus_data('<?= encrypt($data['id_admin']); ?>')">
    ...
    function hapus_data(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location = `data_admin/hapus/${id}`;
            }
        })
    }
</script>
