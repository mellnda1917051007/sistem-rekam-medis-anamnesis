...
<body>
    <a href="<?=base_url("petugas/data_anamnesis/tambah")?>">
        <?php btn_tambah('Tambah'); ?>
    </a>
    <?php /*
    <a target="blank" href="cetak.php?berdasarkan=data_anamnesis&jenis=xls&pakaiperperiode=<?php echo $status_pakaiperperiode; ?>">
        <?php btn_export('Export Excel'); ?>
    </a>

    <a target="blank" href="cetak.php?berdasarkan=data_anamnesis&jenis=print&pakaiperperiode=<?php echo $status_pakaiperperiode; ?>">
        <?php btn_cetak('Cetak'); ?>
    </a>
     */
    ?>
    ...
    <div style="overflow-x:auto;">
        <table <?php tabel(100, '%', 1, 'left'); ?>>
            <tr>
                ...
                <th>No</th>
                <th>Id Anamnesis</th>
                <th align="center" class="th_border cell">Keadaan </th>
            ...
            <tbody>
                ...
                foreach ($data_anamnesis as $data) { ?>
                    <tr class="event2">
                        <td class="th_border cell" align="center" width="200">
                            <table border="0">
                                <tr>
                                    <td>
                                        <a href="<?php index(); ?>/detail?proses=<?= encrypt($data['id_anamnesis']); ?>">
                                            <?php btn_detail('Detail'); ?></a>
                                    </td>
                                    <td>
                                        <a href="<?php index(); ?>/edit?proses=<?= encrypt($data['id_anamnesis']); ?>">
                                            <?php btn_edit('Edit'); ?></a>
                                    </td>
                                    <td>
                                        <a onclick="hapus_data('<?= encrypt($data['id_anamnesis']); ?>')">
    ...
    function hapus_data(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location = `data_anamnesis/hapus/${id}`;
            }
        })
    }
</script>
