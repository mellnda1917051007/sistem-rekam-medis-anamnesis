...
use App\Models\DataAnamnesis as ModelsDataAnamnesis;

class DataAnamnesisRepo
{
    private static $model = ModelsDataAnamnesis::class;

    /**
     * @param $berdasarkan
     * @param $isi
     * @return ModelsDataAnamnesis
     */
    ...
    {
        $db = db_connect();
        return $db->query("desc data_anamnesis")->getResultArray();
    }
    ...
    {
        $model = new self::$model();
        $model->where("id_anamnesis", $proses);
        return $model->first();
    }
    ...
    public static function cetak_berdasarkan_tanggal($berdasarkan, $tanggal1, $tanggal2)
    {
        ...
        return $model->findAll();
            ...
            public static function get_data_anamnesis ($id_anamnesis) {
            $model = new \App\Models\DataAnamnesis();
            $model->where("id_anamnesis", $id_anamnesis);
            return $model->first();
        }				
